...
export default function College() {
  ...
  // Log selected language changes for debug
  useEffect(() => {
    console.log("Selected language changed to:", selectedLang);
  }, [selectedLang]);
  ...
  const callChatApi = async (query, extra = {}) => {
    try {
      ...
      if (extra?.files && extra.files.length > 0) {
        ...
        res = await fetch(CHAT_API, { method: "POST", body: fd });
      } else {
  ...
  const shouldShowStreamMenu =
    messages.length === 2 &&
    !selectedStream &&
    (messages[1]?.text?.toLowerCase().includes("hi") ||
      messages[1]?.text?.toLowerCase().includes("hello") ||
      messages[1]?.text?.toLowerCase().includes("hii"));
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-50 to-white">
      ...
      {selectedStream && (
        <div className="fixed bottom-6 left-6 w-80 bg-white shadow-xl rounded-lg p-4 z-40 border">
          ...
          <div className="mt-3 space-y-2 max-h-64 overflow-y-auto">
            {courses[selectedStream].map((c, i) => (
              <div key={i} className="p-2 bg-gray-50 rounded border flex justify-between items-center">
                <div>{c}</div>
